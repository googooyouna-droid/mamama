  ...
  Future<void> _saveDiary() async {
    try {
      ...
      if (emotion != null && text != null) {
        ...
        // 현재 일기 엔트리 가져오기 또는 생성
        final currentEntry = _currentEntry ?? DiaryEntry(
          date: widget.date,
          child: const DiarySection(lastModified: DateTime.now()),
          parent: const DiarySection(lastModified: DateTime.now()),
          calendarEmoji: '🌱',
          createdAt: DateTime.now(),
          updatedAt: DateTime.now(),
        );

        // 업데이트된 섹션으로 일기 엔트리 생성
        final updatedEntry = currentEntry.copyWith(
          child: role == 'child' ? sectionData : currentEntry.child,
          parent: role == 'parent' ? sectionData : currentEntry.parent,
          updatedAt: DateTime.now(),
        );

        // 네트워크 상태에 따라 저장 방식 결정
        if (await _isOnline()) {
          // 온라인: Firebase에 직접 저장
          ...
          // 오프라인: 로컬에 저장
          await appStateProvider.saveOfflineDiary(widget.date, updatedEntry);
          
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('오프라인에 저장되었습니다. 연결 시 자동 업로드됩니다.'),
          ...
          Expanded(
            child: Column(
              children: [
                ...
                Expanded(
                  flex: 1,
                  child: PhotoAttachment(
                    date: widget.date,
                    section: title == '자녀의 마음' ? 'child' : 'parent',
                    canEdit: canEdit,
                  ),
                ),
                ...
                Expanded(
                  ...
                  child: CommentSection(
                    date: widget.date,
                    targetSection: title == '자녀의 마음' ? 'child' : 'parent',
                  ),
                ),
              ],
            ),
